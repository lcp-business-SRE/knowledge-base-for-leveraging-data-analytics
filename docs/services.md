...
- 料金を最適化は程々までにしたほうが良い。
  - ファイルサイズ・アクセス頻度・保存期間により一番安いソリューションが変化する。定期的に運用を見直したり、ファイル転送回数削減やファイル圧縮を試したりとコスト削減のアプローチ方法が多い。しかし、それによって削減できる費用がそこまで大きくない場合もあるのでS3 Intelligent-Tieringの自動分類に任せたりとかも考えられる。
