...
- 多種大量のデータをデータレイクなどの保管場所に保存する
  - データ形式は自由。
- 保存前にJsonからのフォーマット変換処理が可能
  - [Apache Parquet](https://parquet.apache.org/) ・ [Apache ORC](https://orc.apache.org/)の列指向データ形式に変換可能。これによりクエリ速度の向上が見込める。
...
- 詳細な変換が難しい
- 投げ込む際に書式を整える必要性がある
  - Bufferで投げ込んでもちゃんと解釈してくれるし、Jsonをそのままでも使える。しかし、吐き出されるテキストが異なるので後で分析する際に障壁となることがある。
