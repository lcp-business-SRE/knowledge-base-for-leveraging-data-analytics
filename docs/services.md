...
| --- | --- | --- | --- | --- | --- | --- | --- |
|  概要   |  標準的なデータ向け   |   アクセス頻度が低いデータ向け  |   アクセス頻度がさらに低いデータ向け  |  年数回未満レベルのアクセスで、急を要しないデータ向け   |再作成可能なデータ向け | 年数回レベルのアクセスするデータ  | アクセス頻度等でストレージを振り分ける |
|  データ容量に対する価格(/GB)   |  0.025   |  0.0138   |   0.005  |   0.002  | 0.011 |0.0045  |0.025～0.005(高頻度～アーカイブ)|
|  データ取り出しに対する価格(/1000リクエスト)   |  0   |   0  |  0   |  0.1142/0.025  (標準/大容量) | 0| 11.00/0.0571/0 (迅速/標準/大容量)|0 |
|  データ取り出しに対する価格(/GB)   |  0   |   0.01  |  0.03   |  0.022/0.005  (標準/大容量) | 0.01 | 0.033/0.011/0 (迅速/標準/大容量)|0 |
|  PUT、COPY、POST、LIST リクエスト(/1000リクエスト)   |   0.0047  |   0.01  |  0.02   |   0.065  | 0.01| 0.03426 |0|
|  GET、SELECT、他のすべてのリクエスト (/1000リクエスト)   |  0.00037 |   0.001  |  0.01   |  0.00037 | 0.001| 0.00037 |0|
...
|  備考   |     |     |     |  決済データなどの法的に長期保存を要求されるデータ向け   | AZ1つにしか保存されないため、安価。復帰可能なものや消えてもいいもの向け| アクセス頻度が低く急がないデータ向け。素早く取り出そうとするほど料金が跳ね上がる。 |128KB以上のサイズのデータに関してはモニタリングおよびオートメーションが行われ、1000件当たり0.0025かかる。</br>一番使用頻度が低いデータを取得する場合に、優先リクエストを行うと課金される。</br>128KB未満のデータは振り分け対象外|
...
- Glacierは大量の軽量オブジェクトを保存したり、短期間で削除するなどの運用をすると通常のストレージよりも高額になる罠がある。
  - 軽量オブジェクト保存時の問題
...
- 料金を最適化は程々までにしたほうが良い。
  - ファイルサイズ・アクセス頻度・保存期間により一番安いソリューションが変化する。定期的に運用を見直したり、ファイル転送回数削減やファイル圧縮を試したりとコスト削減のアプローチ方法が多い。しかし、それによって削減できる費用がそこまで大きくない場合もある。S3 Intelligent-Tieringの自動分類に任せるのが無難。
