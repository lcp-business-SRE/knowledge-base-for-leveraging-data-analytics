...
データ収集ツールの一つ。様々なツールから情報をAPI経由で取得し、指定した場所にそれを格納する。
収集元・送信先ともにコネクタと呼ばれるものを通じて結合する。

#### 2. サービスの特徴

- EL(T)ツールと名乗っている
  - 集めて保存するツール。Transformは分離されている。
- Elastic License 2.0版と、クラウドサービスでの提供の2種類のサービスがある。
- データの収集元や送信先とはコネクタでつながる。
  - データ収集
    - コネクタビルダー
      - 公式が入力元毎に100種類以上提供している。[一覧](https://airbyte.com/connectors?connector-categories)
        - 各種DBやアナリティクスサービスに対応
      - GUIで容易に設定が可能
      - 必要事項を入力していけばコネクタが出来上がる。
    - カスタムコネクタ
      - コネクタビルダーで対応できないデータソースに対するカスタム可能なコネクタ
      - [Low-code connector](https://docs.airbyte.com/platform/connector-development/config-based/low-code-cdk-overview)
        - Yamlファイルでコネクタを記述する。
      - [Python CDK](https://docs.airbyte.com/platform/connector-development/cdk-python/)
        - Pythonを用いてコネクタを記述する。とても自由度が高い。
      - これらのカスタムコネクタはクラウドサービス側では利用できない。
  - データ配信
    - 収集したデータをコネクタ経由で指定したところに送信できる。[一覧](https://airbyte.com/connectors?connector-type=Destinations)

#### 3. 価格

- 無料版あり
  - ライセンスはElastic License 2.0で配布されている。(マネージドサービス化しなければよい)
  - Docker+Kubernetesで配られているので、EC2等でホストする必要がある。
- Cloud版もあるが割高感が否めない
  - 入力 $15 / millio
  - 処理データ量  $10 / GB
...
- Dockerファイルを使うので一発で構築が終わる
- GUIで容易にAPIとの接続が可能
- 保存先に置くデータは設定により形式を揃えられる。
  - 自動でもタイムスタンプとデータの中身程度にはカラムにしてくれるようだった
- コネクタの追加が期待できる
...
- ソースコードを書きたくないし、有償の物も使いたくない場合はいい選択肢になる。

#### 8. 用途についての所感

- どこまでデータ収集用のコネクタが必要なのかの実感がわかないため試用してみるしかないか。
- クラウドサービス間を跨いだ処理をしやすいのは好印象

#### 9. 備考

- ライセンスの関係でクラウドサービスがマネージドで出してくることはないと思われる。
- オンプレで試してみて、使い勝手が良さそうなら本格的に使うといった方針がいいか？
